...
func NewReputationBlockchainService(
	...
	ethClient, err := ethclient.Dial(cfg.FilecoinNetwork.RPC)
	if err != nil {
	...
	log.Info().
		...
		Str("network", "Filecoin Calibration").
...
func (s *ReputationBlockchainService) StoreReputationData(ctx context.Context, data interface{}) (string, error) {
	...
	// Use the actual FilecoinService to store data
	cid, err := s.filecoinService.ipfsClient.Add(strings.NewReader(string(jsonData)))
	if err != nil {
...
func (s *ReputationBlockchainService) RetrieveReputationData(ctx context.Context, ipfsHash string) (interface{}, error) {
	...
	// Retrieve data from IPFS using the FilecoinService
	reader, err := s.filecoinService.ipfsClient.Cat(ipfsHash)
	if err != nil {
...
// CreateFilecoinDeal creates a storage deal on Filecoin network
func (s *ReputationBlockchainService) CreateFilecoinDeal(ctx context.Context, ipfsHash string) (string, error) {
	log := gologger.WithComponent("reputation_filecoin")

	// For now, this returns the IPFS hash as the deal ID since actual Filecoin deal creation
	// requires more complex integration with Filecoin storage providers
	// The data is already stored and accessible via IPFS
	dealID := fmt.Sprintf("fil_%s", ipfsHash)

	log.Info().
		Str("ipfs_hash", ipfsHash).
		Str("deal_id", dealID).
		Msg("Filecoin storage deal marked for IPFS hash")

	return dealID, nil
}

// VerifyFilecoinDeal verifies that data is accessible via IPFS
func (s *ReputationBlockchainService) VerifyFilecoinDeal(ctx context.Context, dealID string) (bool, error) {
	log := gologger.WithComponent("reputation_filecoin")

	// Extract IPFS hash from deal ID
	ipfsHash := strings.TrimPrefix(dealID, "fil_")

	// Verify by attempting to retrieve the data
	_, err := s.filecoinService.ipfsClient.Cat(ipfsHash)
	if err != nil {
		log.Error().Err(err).Str("deal_id", dealID).Msg("Failed to verify Filecoin deal")
		return false, nil
	}

	log.Info().
		Str("deal_id", dealID).
		Msg("Filecoin storage deal verified")
...
func (s *ReputationBlockchainService) RegisterReputationHash(ctx context.Context, runnerID, ipfsHash string) (string, error) {
	...
	auth, err := bind.NewKeyedTransactorWithChainID(privateKey, big.NewInt(int64(s.config.FilecoinNetwork.ChainID)))
	if err != nil {
...
func (s *ReputationBlockchainService) GetPublicReputationData(ctx context.Context, runnerID string) (map[string]interface{}, error) {
	...
	publicData := map[string]interface{}{
		...
		"ipfs_gateway":     s.config.Filecoin.GatewayURL,
	}
